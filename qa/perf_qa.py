...
def run_case(browser,name,viewport,dpr):
    ...
    state=page.evaluate("""()=>{const v=document.querySelector('video.hero-art');return {scrollY,width:innerWidth,height:innerHeight,dpr:devicePixelRatio,heroP:getComputedStyle(document.documentElement).getPropertyValue('--hero-p').trim(),videoPaused:v?.paused,videoTime:v?.currentTime,videoReady:v?.readyState,fontOk:document.fonts?.check('16px Sora')}}""")
...
with sync_playwright() as p:
    ...
    reports=[run_case(browser,"desktop-1440",{"width":1440,"height":1000},1),run_case(browser,"mobile-390",{"width":390,"height":844},3)]
    browser.close()
